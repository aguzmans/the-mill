...
import { executePlan, makeCtx, WorkflowError, type RunEvent, type Ctx, type NodeFn } from "@mill/sdk";
...
export async function runWorkflow(job: Job, onEvent?: (e: RunEvent) => void): Promise<ExecResult> {
  const events: RunEvent[] = [];
  const sink = (e: RunEvent) => { events.push(e); onEvent?.(e); };
  const t0 = performance.now();
  try {
    const { result, statuses } = await runOne(job.projectDir, job.workflow, job.input, job.secrets ?? {}, sink, job.revision);
    ...
    switch (node.kind) {
      ...
      case "loop": {
        ...
        for (let i = 0; i < arr.length; i++) {
          state.index = i;
          state.item = arr[i];
          results.push(bodyFn ? await bodyFn(arr[i], ctx) : (await runOne(projectDir, resolveCallTarget(node.call!.ref), arr[i], secrets, sink, revision)).result);
        }
        output = results;
        break;
      }
...
export class SubprocessExecutor implements Executor {
  ...
  async execute(job: Job): Promise<ExecResult> {
    ...
    const proc = Bun.spawn(
      ...
      { stdout: "pipe", stderr: "pipe", env: { ...process.env, MILL_SECRETS: JSON.stringify(job.secrets ?? {}) } },
    );
    let timedOut = false;
